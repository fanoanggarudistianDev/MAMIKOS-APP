    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-8 items-start">
        ...
        <div class="xl:col-span-2 bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
            <div class="p-4 border-b border-slate-100 flex items-center justify-between">
                ...
                <div class="text-xl font-bold text-emerald-600">Rp {{ number_format($total_pendapatan ?? 0, 0, ',', '.') }}</div>
            </div>
                ...
                <tbody class="divide-y divide-slate-100 font-medium text-slate-700">
                    ...
                    <tr class="hover:bg-slate-50/50 transition">
                        ...
                        <td class="py-3.5 px-4">
                            <form action="{{ route('pembayaran.destroy', $pembayaran->id) }}" method="POST" onsubmit="return confirm('Hapus pembayaran ini?');">
                                @csrf
                                @method('DELETE')
                    ...
                    @empty
                    <tr><td colspan="4" class="py-8 text-center text-slate-400">Belum ada riwayat jurnal keuangan.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
